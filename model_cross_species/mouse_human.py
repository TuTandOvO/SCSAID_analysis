...
WORKDIR = r"/gpfsdata/home/renyixiang/SkinDB/cross_species/mouse_train_human_test"
...
ROC_EXPORT_MODE = "all_pruned"   # "all_pruned" | "panel_only"
...
OUT_METRICS_MOUSE = os.path.join(WORKDIR, "mouse_internal_metrics.json")
OUT_METRICS_HUMAN = os.path.join(WORKDIR, "human_external_metrics.json")
OUT_REPORT_MOUSE = os.path.join(WORKDIR, "mouse_internal_classification_report.txt")
OUT_REPORT_HUMAN = os.path.join(WORKDIR, "human_external_classification_report.txt")

OUT_BUNDLE = os.path.join(WORKDIR, "cross_species_bundle.json")
OUT_HUMAN_SCORES = os.path.join(WORKDIR, "human_external_scores.tsv")
OUT_MOUSE_PANEL_STATS = os.path.join(WORKDIR, "mouse_panel_stats.tsv")
...
def corr_on_train_subsample_subset(lg, tr_rows: np.ndarray, gene_idx: np.ndarray, max_cells: int, block_rows: int):
    ...
    for s, e in iter_ranges(n_rows, block_rows):
        chunk_rows = rows[s:e]
...
def export_final_coefficients(clf, panel_mouse):
    feat_names = list(panel_mouse.astype(str)) + AGE_STAGE_COLS + ["sex"]
...
def load_mouse_human_ortholog_candidates(mouse_var_names, human_var_names, ortholog_tsv):
    ...
    orth = orth.drop_duplicates(subset=["mouse_gene", "human_gene"]).copy()

    mouse2human = dict(zip(orth["mouse_gene"], orth["human_gene"]))
    human2mouse = dict(zip(orth["human_gene"], orth["mouse_gene"]))

    candidate_mask = np.isin(mouse_var_names, orth["mouse_gene"].to_numpy(dtype=object))
    candidate_pos = np.where(candidate_mask)[0].astype(np.int32)

    return orth, mouse2human, human2mouse, candidate_pos
...
def main():
    ...
    with log_step("LOAD ORTHOLOG"):
        orth, mouse2human, human2mouse, candidate_pos = load_mouse_human_ortholog_candidates(
            var_m,
            var_h,
            ORTHOLOG_TSV,
        )
    ...
    with log_step("BUILD MOUSE TASK"):
        ...
        if groups_m is not None and USE_GROUP_SPLIT:
            tr_i, te_i = stratified_group_split_local(
                y_m,
                groups_m.to_numpy(dtype=object),
                TEST_SIZE,
                RANDOM_STATE,
            )
            groups_arr_m = groups_m.to_numpy(dtype=object)
        else:
        ...
        tr_rows_u = task_rows_m[tr_i].astype(np.int64)
        te_rows_u = task_rows_m[te_i].astype(np.int64)
        y_tr, y_te = y_m[tr_i], y_m[te_i]

        print_counts("mouse_train", y_tr)
        print_counts("mouse_test", y_te)

    with log_step("EFFECT SCORE"):
        score_all = supervised_effect_score_all_genes(M["lg"], M["n_genes_z"], tr_rows_u, y_tr)
        cand_scores = score_all[candidate_pos]

        pd.DataFrame({
            "mouse_gene": var_m[candidate_pos].astype(str),
            "effect_score": cand_scores,
            "human_gene": [mouse2human[g] for g in var_m[candidate_pos].astype(str)],
        }).sort_values("effect_score", ascending=False).to_csv(
            OUT_CANDIDATE_SCORES, sep="\t", index=False
        )

    with log_step("CORR PRUNING"):
        ...
        pruned_local, pruned_names = prune_by_abs_corr_greedy(
            var_m[candidate_pos].astype(object),
            cand_scores,
            corr,
            thr=PRUNE_ABS_CORR,
        )
        ...
        pd.DataFrame({
            "mouse_gene": pruned_names_arr,
            "mouse_gene_index": pruned_global,
            "human_gene": [mouse2human[g] for g in pruned_names_arr.astype(str)],
        }).to_csv(OUT_PRUNING_MAP, sep="\t", index=False)

    with log_step("BUILD X MEMMAP"):
        if os.path.exists(XMM_PATH):
            os.remove(XMM_PATH)

        n_task_m = int(task_rows_m.size)
        Xmm = np.memmap(XMM_PATH, dtype=np.float32, mode="w+", shape=(n_task_m, int(pruned_global.size)))
        ...
        for s, e in iter_ranges(n_task_m, 2000):
    ...
    with log_step("STABILITY SELECTION"):
    ...
    with log_step("BUILD TOP PANEL"):
        panel_mouse = df_stab.head(PANEL_K)["gene"].to_numpy(dtype=object)
        panel_mask = np.isin(pruned_names_arr, panel_mouse)
        ...
        panel_human = np.array([mouse2human[g] for g in panel_mouse], dtype=object)
        pd.DataFrame({
            "mouse_gene": panel_mouse,
            "human_gene": panel_human,
        }).to_csv(OUT_PANEL, sep="\t", index=False)

        df_stab.head(PANEL_K).to_csv(OUT_MOUSE_PANEL_STATS, sep="\t", index=False)

    with log_step("FINAL MOUSE MODEL"):
        ...
        X_tr = np.concatenate(
            [
                Xg_tr,
                age_mat_m[tr_i, :].astype(np.float32),
                sex_num_m[tr_i].reshape(-1, 1).astype(np.float32),
            ],
            axis=1,
        )
        X_te = np.concatenate(
            [
                Xg_te,
                age_mat_m[te_i, :].astype(np.float32),
                sex_num_m[te_i].reshape(-1, 1).astype(np.float32),
            ],
            axis=1,
        )
        ...
        rep_m = classification_report(y_te, y_pred_in, target_names=["Class0", "Class1"], digits=6)
        ...
        export_final_coefficients(clf, panel_mouse)

        if EXPORT_ROC_DATA:
            if ROC_EXPORT_MODE == "panel_only":
                save_curves(y_te, y_proba_in, os.path.join(WORKDIR, "mouse_internal_panel"))
            else:
                save_curves(y_te, y_proba_in, os.path.join(WORKDIR, "mouse_internal_all_pruned"))
    ...
    with log_step("BUILD HUMAN TASK"):
        ...
        var_h_to_pos = {g: i for i, g in enumerate(var_h.tolist())}
        panel_human_pos = np.array([var_h_to_pos[g] for g in panel_human], dtype=np.int32)

        panel_mean = mean[panel_cols]
        panel_std = std[panel_cols]

    with log_step("HUMAN EXTERNAL VALIDATION"):
        y_proba_ext = np.zeros(task_rows_h.size, dtype=np.float32)

        for s, e in iter_ranges(task_rows_h.size, EXT_CHUNK):
            rows_u = task_rows_h[s:e]
            Xb_full = fill_dense_block_for_row_list(H["lg"], rows_u, H["n_genes_z"])

            Xg = Xb_full[:, panel_human_pos]
            ...
            X_ext = np.concatenate(
                [
                    ...
                    age_mat_h[s:e, :].astype(np.float32),
                    sex_num_h[s:e].reshape(-1, 1).astype(np.float32),
                ],
                axis=1,
            )
        ...
        external_acc = float(accuracy_score(y_h, y_pred_ext))
        external_auc = float(roc_auc_score(y_h, y_proba_ext))
        rep_h = classification_report(y_h, y_pred_ext, target_names=["Class0", "Class1"], digits=6)
        ...
        save_json(
            {
                ...
                "auc": external_auc,
                "panel_k": int(panel_cols.size),
                "n_human_test": int(task_rows_h.size),
                "n_repeats": N_REPEATS,
                "threads": os.environ.get("OMP_NUM_THREADS", None),
            },
            OUT_METRICS_HUMAN,
        ...
        if EXPORT_ROC_DATA:
            save_curves(y_h, y_proba_ext, os.path.join(WORKDIR, "human_external"))

    pd.DataFrame({
        "human_task_row": np.arange(task_rows_h.size, dtype=np.int64),
        "human_zarr_row": task_rows_h,
        "human_orig_obs_index": H["obs_orig_index"][task_rows_h],
        "y_true": y_h,
        "y_proba": y_proba_ext,
        "y_pred": y_pred_ext,
    }).to_csv(OUT_HUMAN_SCORES, sep="\t", index=False)
